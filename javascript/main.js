...
;(function($) {
   ...
   'use strict'
        ...
        window.onload = function () {
            var counters = document.querySelectorAll(".count-down");

            counters.forEach(function (cd) {
                var expiryStr = cd.getAttribute("data-expiry");
                if (!expiryStr) return;

                var expiry = new Date(expiryStr.replace(" ", "T")); // fallback VS2012 safe
                if (isNaN(expiry.getTime())) {
                    console.log("Invalid date format:", expiryStr);
                    return;
                }

                var d = cd.querySelector(".days");
                var h = cd.querySelector(".hours");
                var m = cd.querySelector(".mins");
                var s = cd.querySelector(".secs");

                function update() {
                    var now = new Date().getTime();
                    var diff = expiry.getTime() - now;
    ...
    // Product Detail Tabs
        var tabProductDetail = function () {
            ...
            $('.flat-product-content').each(function () {
                ...
                // by default: pehla li active + pehla row show
                $content.find('ul.product-detail-bar li').removeClass('active').first().addClass('active');
                $content.find('.container > .row').hide().first().show();

                // tab click
                $content.find('ul.product-detail-bar li').on('click', function (e) {
                    e.preventDefault();
                    var liActive = $(this).index();
            ...
            // Style2 Product Detail
            $('.flat-product-content.style2').each(function () {
                var $content = $(this);

                // default
                $content.find('ul.product-detail-bar li').removeClass('active').first().addClass('active');
                $content.find('.col-md-12 > .row').hide().first().show();
